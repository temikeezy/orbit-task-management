...
class OTM_Widget_Overall extends WP_Widget {
    public static function register() { add_action('widgets_init', function(){ register_widget(__CLASS__); }); }
    public function __construct() { parent::__construct('otm_overall_leaderboard', 'OTM Overall Leaderboard', ['description' => 'Shows the all-time leaderboard']); }
    public function form($instance) {
        $title = esc_attr(isset($instance['title']) ? $instance['title'] : 'Overall Standings');
    ...
    public function widget($args, $instance) {
        ...
        echo do_shortcode('[otm_leaderboard week="all" limit="'.intval(isset($instance['limit'])?$instance['limit']:10).'"]');
